...
def test_get_pool_cov_overview___given_simple_parameters___returns_expected_data(expert_client: ExpertClient) -> None:
    # Act
    indicator_overview = expert_client.get_pool_cov_overview(granularity='Day', search='Business')

    # Assert
    assert any(indicator_overview.detailed_pool_cov_information.name.str.contains('Business'))


def test_get_pool_cov_overview___given_implausible_parameters___raises_error(expert_client: ExpertClient) -> None:
    # Act & Assert
    with pytest.raises(ValueError, match='No data found with the specified parameters'):
        expert_client.get_pool_cov_overview(granularity='Day', search='implausiblekeyword')


def test_checkin_pool___get_valid_pool_cov_ids___creates_version(expert_client: ExpertClient) -> None:
    # Arrange
    indicator_overview = expert_client.get_pool_cov_overview(granularity='Day')
    requested_pool_covs = []
    ...
    # Act
    checkin_result = expert_client.check_in_pool_covs(requested_pool_covs=requested_pool_covs)
...
def test_query___get_valid_query___creates_expected_result(expert_client: ExpertClient) -> None:
    # Arrange
    indicator_overview = expert_client.get_pool_cov_overview(granularity='Day')
...
def test_query___rerun_valid_query___creates_expected_result(expert_client: ExpertClient) -> None:
    # Arrange
    indicator_overview = expert_client.get_pool_cov_overview(granularity='Day')
...
def test_query___get_invalid_query___raises_error(expert_client: ExpertClient) -> None:
    # Arrange
    indicator_overview = expert_client.get_pool_cov_overview(granularity='Day')
...
def test_create_pool_cov_definitions___called____returns_expected_list(expert_client: ExpertClient) -> None:
    # Arrange
    indicator_overview = expert_client.get_pool_cov_overview(granularity='Day')
    queried_overview = indicator_overview.query(expr='name.str.contains("ferien")')
